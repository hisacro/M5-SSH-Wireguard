 ...
 *   Fn + T        →  Toggle TomThumb Font (60/80 Columns)
...
struct Settings {
    ...
    int  TTfToggle; // experimental TomThumb
    int  TTfRows;   // experiment no.of TomThumb rows
};
...
Settings g_cfg = { 60, 0, 0, 128, 0, 1, true, false, 22, true, 0, 0, 1 };
...
void loadSettings() {
    ...
    while (f.available()) {
        ...
        // experimental (not saved)
        else if (k=="TTf_toggle")     g_cfg.TTfToggle        = (v==1)?1:0;  // TomThumb
        else if (k=="TTf_rows")       g_cfg.TTfRows          = (v==2)?2:1;  // 80 or 60 chars
    }
    f.close();
    M5Cardputer.Display.setBrightness(g_cfg.brightness);
}
            ...
            if (now - g_lastKey >= DEBOUNCE_MS) {
                ...
                if (isFn()) {
                    for (auto hid : st.hid_keys) {
                        ...
                        // Toggle_TomThumb
                         if (a == 't') {
                            // toggles between default font, TT with 60 or 80 cols.
                            if (g_cfg.TTfToggle == 0) {
                              g_cfg.TTfToggle = 1;
                            } else if (g_cfg.TTfRows == 1) {
                              g_cfg.TTfToggle = 1;
                              g_cfg.TTfRows = 2;
                            } else {
                              g_cfg.TTfToggle = 0;
                            ...
                            // saveSettings(); // needs another termCols in connectSSH to make it work properly.
                            TOP  = title_tog(); 
                            tCols = termCols(); tRows = termRows();
                            scrollTop = 0; scrollBot = tRows - 1;
                            clearBuf(0); clearBuf(1);
